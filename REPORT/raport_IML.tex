...
\textit{Measured impact (to be completed):}
\begin{itemize}
    \item Validation accuracy change: $\pm$ XX \%
    \item Macro F1-score change: $\pm$ XX
    \item Generalization gap (train–val) change: $\pm$ XX \%
    \item Convergence speed (epochs to plateau): $\pm$ XX epochs
\end{itemize}
...
\paragraph{Optimizer Comparison: SGD vs. AdamW}
...
\textit{Measured impact (to be completed):}
\begin{itemize}
    \item Initial convergence speed (epochs to reach XX\% accuracy): SGD XX / AdamW XX
    \item Final validation accuracy difference: AdamW $-$ SGD = $\pm$ XX \%
    \item Macro F1-score difference: AdamW $-$ SGD = $\pm$ XX
    \item Training stability (std. dev. of val accuracy): SGD XX / AdamW XX
    \item Mean epoch time difference: AdamW $-$ SGD = $\pm$ XX ms
\end{itemize}
...
\textit{Measured impact (to be completed):}
\begin{itemize}
    \item Epochs to peak validation accuracy: OneCycleLR XX / CosineAnnealingLR XX
    \item Peak validation accuracy difference: CosineAnnealingLR $-$ OneCycleLR = $\pm$ XX \%
    \item Validation loss smoothness (variance across epochs): OneCycleLR XX / CosineAnnealingLR XX
    \item Generalization gap (train–val accuracy): OneCycleLR XX / CosineAnnealingLR XX
    \item Total training time difference: CosineAnnealingLR $-$ OneCycleLR = $\pm$ XX min
\end{itemize}
...
\textit{Measured impact (to be completed):}
\begin{itemize}
    \item Mean predictive variance (early vs late epochs): $\downarrow$ XX \%
    \item Correlation between uncertainty and errors: XX
    \item Deterministic accuracy change: $\pm$ XX \%
\end{itemize}
...
Xavier uniform initialization in the AAMSoftmax head is intended to ensure balanced variance propagation and prevent early saturation of angular margins. This supports stable early training and consistent classifier convergence.

\textit{Measured impact (to be completed):}
\begin{itemize}
    \item Initial training loss (epoch 1) change: $\pm$ XX
    \item Epochs to stable margin separation: $\pm$ XX epochs
    \item Final validation accuracy change: $\pm$ XX \%
\end{itemize}
...
Michał Kozicki &
    ...
    implementation of the hybrid CNN-RNN with AASM head, and

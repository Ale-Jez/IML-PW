...
# CONFIGURATION
# ============================================================================
MODEL_FILE = "speaker_recognition.onnx"
BANK_FILE = "speaker_backbone_speaker_bank_multi.npy"
...
async def recognize_speaker(audio_path):
    ...
    class_1 = {1: "Alesander", 27: "Mantas", 29: "Michal", 38: "Piotr", 40: "Rafał"}

    is_member = best_idx in class_1

    is_authorized = bool(is_member and best_score >= THRESHOLD)

    name = class_1.get(best_idx, "Unknown")
...
for file_path in audio_files:
    ...
    display_name = result['name'] if result['authorized'] else "idk"

    # Formatting confidence to %
    print(
        f"{fname[:33]:<35} | {result['speaker_id']:<3} | {display_name:<12} | {result['confidence']:.1%}   | {status}")